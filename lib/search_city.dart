// search_city.dart 🌙 Dark Mode Edition
import 'dart:async';
...
class _SearchCityScreenState extends State<SearchCityScreen> {
  ...
  Future<void> _loadSelf() async {
    try {
      ...
      final data = doc.data() ?? {};
      _myCityLower = (data['city_lower'] ?? data['city'] ?? '')
          .toString()
          .trim()
          .toLowerCase();
    } finally {
  ...
  Stream<List<QueryDocumentSnapshot<Map<String, dynamic>>>> _userStream() async* {
    ...
    final q1 = _fire.collection('users').where('city_lower', isEqualTo: _myCityLower);
    final q2 = _fire.collection('users').where('city', isEqualTo: _myCityLower);

    await for (final snap1 in q1.snapshots()) {
      final snap2 = await q2.get();
      final merged = <String, QueryDocumentSnapshot<Map<String, dynamic>>>{};

      for (final d in snap1.docs) merged[d.id] = d;
      for (final d in snap2.docs) merged[d.id] = d;

      // Safe sweep to catch any stray users
      final allUsers = await _fire.collection('users').get();
      for (final d in allUsers.docs) {
        final data = d.data();
        final rawCity = (data['city'] ?? '').toString().trim().toLowerCase();
        if (rawCity == _myCityLower) {
          merged[d.id] = d;
        }
      }

      final filtered = merged.values.where((doc) {
  ...
  Widget build(BuildContext context) {
    ...
    if (_myCityLower == null || _myCityLower!.isEmpty) {
      return const Scaffold(
        ...
        body: _EmptyState(
          title: 'Add your city',
          subtitle: 'Pick your city in Edit Profile to see people around you.',
        ),
      );
    }
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          _TopSearchBar(controller: _searchCtrl, hint: 'Search username in your city'),
          Expanded(
              ...
              builder: (context, snap) {
                ...
                if (docs.isEmpty) {
                  return const _EmptyState(
                    title: 'No users yet',
                    subtitle: 'Be the first from your city to appear here!',
                  );
                }
...
class _TopSearchBar extends StatelessWidget {
  ...
  final String hint;
  const _TopSearchBar({Key? key, required this.controller, required this.hint})
      : super(key: key);
    ...
    return Container(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            const Icon(Icons.people_outline, size: 52, color: Colors.white38),
